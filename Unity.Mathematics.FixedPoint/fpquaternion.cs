...
namespace Unity.Mathematics.FixedPoint
{
    ...
    public struct fpquaternion : IEquatable<fpquaternion>, IFormattable
    {
        ...
        {
            fp dot = Dot(q1, q2);
            return fpmath.acos(fpmath.min(fpmath.abs(dot), fp.one)) * (fp)2;
        }
    ...
    public static partial class fpmath
    {
        ...
        /// <summary>Returns the asin function of a fixed-point value.</summary>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        ...
        { 
            ...
            if (x == fp.one) return PI_OVER_2;
            if (x == -fp.one) return -PI_OVER_2;
            
            return atan(x / sqrt(fp.one - x * x));
        }
